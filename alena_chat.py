...
_EXHAUSTED_TEXT = (
    ...
    "Хочешь продолжать со мной сколько угодно — это Клуб «Манифест»: разговоры "
    "без счёта + эфир раз в неделю + закрытый чат. 990 ₽/мес.\n\n"
    "А если тянет вглубь и лично — сессия 1:1."
)

_EXHAUSTED_VOICE = (
    "Бесплатная встреча у нас уже была — вторых не даю. Хочешь продолжать со "
    "мной сколько угодно — это Клуб «Манифест»: разговоры без счёта, эфир раз "
    "в неделю и закрытый чат. А если тянет вглубь и лично — сессия один на "
    "один. Обе двери — под этим сообщением."
)
...
async def open_shadow_session(target: Message, user, code: str,
    ...
    if video_hook:
        hello = (f"{who} была я — только что, на видео. Вот и познакомились "
                 ...
                 "исчезла, просто думаю над твоими словами. Под этим "
                 "сообщением — мой первый вопрос и подсказки: выбери ту, что "
                 "отзывается, или скажи по-своему. А в конце подведём главное.")
    else:
    ...
    try:
        await target.answer(
            f"Мой вопрос:\n\n«{q}»\n\n"
            "Ответь как тебе удобно 🎙 — или нажми, что ближе:",
...
async def _after_close(message: Message, user, request: str | None = None):
    ...
    if request:
        ...
        if (u_row or {}).get("lead_track") == "T4":
            ...
            flagship = (
                f"Твой настоящий запрос — вот он: «{q}». Такое разматывают не в "
                "переписке. Ты готова — я это вижу по нашему разговору. Поэтому "
                "скажу прямо: возьми его на живую встречу со мной, один на один "
                "— час только про тебя, именно с этим. Если хочешь мягче и "
                "постепенно — есть Клуб: я рядом каждый день, безлимит наших "
                "встреч, эфиры, круг женщин с похожими историями. Обе двери — "
                "под этим сообщением.")
        ...
        teaser = ("Не уходи. Самое важное скажу не текстом — записываю для "
                  "тебя видео, лично. Это займёт несколько минут.")
    ...
    soft = ("Это была твоя бесплатная встреча — больше таких не будет. Захочешь "
            "продолжить — я рядом каждый день в Клубе «Манифест»: без лимита, в "
            "чате и на эфирах. Он только открылся — заходишь в числе первых. "
            "990 в месяц, чтобы не разбираться со всем этим в одиночку. Дверь — "
            "под этим сообщением.")
