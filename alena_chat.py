...
_EXHAUSTED_TEXT = (
    ...
    "Хочешь продолжать со мной — это Клуб «Манифест»: каждое утро моё аудио "
    "«Манифест дня» + живой эфир раз в неделю + закрытый чат круга + мои "
    "письма. 990 ₽/мес.\n\n"
...
_EXHAUSTED_VOICE = (
    ...
    "мной — это Клуб «Манифест»: каждое утро моё аудио «Манифест дня», живой "
    "эфир раз в неделю, закрытый чат круга и мои письма. А если тянет вглубь "
...
async def open_shadow_session(target: Message, user, code: str,
    ...
    if rem is not None and rem <= 0:
        ...
        spent = (f"Твоя Тень — {a['name']}. {a['teaser']} Мы это уже начинали "
                 "разбирать. Продолжить — в Клубе «Манифест»: каждое утро моё "
                 "аудио, живой эфир раз в неделю и закрытый чат круга. Дверь — "
                 "под этим сообщением.")
        if not await send_voice_reply(target, spent, _club_kbd()):
            await target.answer(
                ...
                "каждое утро моё аудио «Манифест дня» + живой эфир раз в неделю + "
                "закрытый чат круга. 990 ₽/мес.\n\n— Алёна",
...
_STALE_NUDGE_TEXT = (
    ...
    "день в Клубе «Манифест»: утреннее аудио от меня, живой эфир раз в неделю, закрытый чат. "
...
_OFFER_FALLBACK_TEXT = (
    "В Клубе «Манифест» продолжим ровно с этого места: каждое утро — моё аудио "
...
async def _offer_kruzhok(bot, chat_id: int, tg_id: int,
    ...
    try:
        ...
        script = (f"{who}. То, что у тебя сегодня открылось — «{q}» — это "
                  ...
                  "полпути. Я собрала Клуб именно для этого: каждое утро я с тобой голосом — "
                  "мой «Манифест дня», раз в неделю живой эфир, и круг женщин, где "
                  "не нужно держать лицо. Продолжим ровно с того места, где мы остановились. "
...
async def _after_close(message: Message, user, request: str | None = None):
    ...
    if request:
        ...
        if (u_row or {}).get("lead_track") == "T4":
            ...
            flagship = (
                ...
                "постепенно — есть Клуб: моё аудио каждое утро, живые эфиры, круг "
    ...
    soft = ("Это была твоя бесплатная встреча — больше таких не будет. Захочешь "
            "продолжить — я рядом каждый день в Клубе «Манифест»: утренние аудио, "
