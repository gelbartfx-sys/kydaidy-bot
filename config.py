...
class Settings(BaseSettings):
    ...
    # Демо Клуба в оффере (Кай 09.07): разбор «The Best Offer» — показать, ЧТО покупает.
    # Пусто = не шлём. Заполнить file_id ролика с НОВЫМ голосом Алёны (старая версия
    # была на HeyGen-TTS — перегенерить, как кружки). Отправляется перед карточкой оффера.
    club_demo_file_id: str = ""
    ...
    # Батч Б → ИСПРАВЛЕНО 10.07 (планёрка воронки): если client_model помечает
    # встречу как ПИК (peak=true), клиентка ушла «на нерве» — это момент ПРОЖИВАНИЯ,
    # а НЕ продажи. Порог тишины на пике ВЫШЕ обычного (не ниже!): даём воздух, не
    # долбим оффером. Баг #32: на пике порог падал до 7 мин → продажа выстрелила за
    # 80с до органического «а куда дальше». Теперь пик = ЗАМЕДЛЯЕТ (35 = 20 + запас 15).
    stale_nudge_peak_minutes: int = 35

    # ── Воронка v2 (планёрка 10.07): лестница Ханта + гейт квалификации ────────
    # ВСЕ ФЛАГИ OFF ПО УМОЛЧАНИЮ — на деплое поведение байт-в-байт как сейчас, пока
    # Кай не включит в Render env. Раскатка последовательная (см. docs/funnel-
    # rebuild-TZ-2026-07-10.md): сначала гейт по стадии, потом ступени, потом сцепка.
    #
    # Гейт продажи: не показываем цену/кнопку Клуба, пока лид не квалифицирован в
    # тёплого (purchase_stage). FAIL-OPEN: None/ошибка/флаг-off → разрешить (не душим
    # продажи). Whitelist (Кай/Алёна) обходит гейт всегда.
    purchase_stage_gate_enabled: bool = False
    # Требовать пройденную ступень сравнения (событие hunt_comparison_shown) как
    # условие оффера. OFF, пока блок 2 не в проде (иначе гейт душит без причины).
    comparison_step_required: bool = False
    # Ступени лестницы Ханта (два раздельных флага — можно катать/A-B по одному).
    # Блок 2: мост-сравнение вариантов в фазе give_shift (без цены/названий).
    hunt_stage2_enabled: bool = False
    # Блок 3: выбор двери (Клуб/1:1) по ТЕМПЕРАТУРЕ (не архетипу) перед ценой.
    # Холодная → всегда Клуб; 1:1 только горячим жёстким одиночкам (R/Q/H/C).
    hunt_stage3_enabled: bool = False
    # Кто обходит гейт всегда (Кай + Алёна для теста боем). CSV tg_id в env при нужде.
    purchase_gate_whitelist: str = "6271776494,680319075"

    # ── Батч Б: мёртвая встреча закрывается С ОФФЕРОМ ─────────────────────────
    # Встреча, где нудж УЖЕ слали (nudged_at NOT NULL), turns ≥ 2 и человек молчит
    # ещё дольше — не бросаем в тишине: закрываем ВДОГОНКУ полным оффер-путём
    # (кружок/карточка + followup-серия), иначе лид теряется навсегда (аудит 06.07).
    dead_session_minutes: int = 30     # молчание после наджа до закрытия с оффером
    dead_session_tick_min: int = 5     # как часто джоб ищет мёртвые встречи
