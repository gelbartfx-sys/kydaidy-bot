...
async def _prompt_shadow_photo(message: Message, dist: str):
    ...
    await message.answer(
        ...
        f"{a['tag']}\n\n"
        "Как всё будет — по шагам:\n"
        "1️⃣ Пришли мне *одно своё фото* (лицо хорошо видно) — я нарисую тебя "
        "акварелью в образе твоей Тени и соберу твой архетипический профиль.\n"
        "2️⃣ Покажу тебе кое-что личное про твою Тень.\n"
        "3️⃣ И дальше — наша *пробная сессия*: одна, бесплатная, как настоящая "
        "встреча один на один. Найдём, что стоит за твоей Тенью — твой настоящий "
        "запрос. Отвечать можно текстом или голосовыми, как тебе удобнее.\n\n"
        "Начнём с фото. _Оно нужно только для рисунка — я его не храню._",
        parse_mode="Markdown",
    )
