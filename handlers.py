...
async def _prompt_shadow_photo(message: Message, dist: str):
    ...
    await message.answer(
        ...
        "Пара слов, чтобы ты знала, с кем говоришь: я не раздаю формул счастья и "
        "по голове гладить не буду. Я сама когда-то решила, что близость и семья — "
        "не для меня. Думала, это моя сила. Оказалось — броня, за которой я "
        "пряталась. Свою сняла — теперь рядом с теми, кто снимает свою.\n\n"
        f"Твоя ведущая Тень — *{a['name']}* _({a['too']})_. {a['tag']}\n\n"
        "Как пойдём: пришли мне *одно своё фото*, где хорошо видно лицо — нарисую "
        "тебя акварелью в образе твоей Тени и соберу твой профиль. Потом покажу "
        "кое-что личное про эту Тень. А дальше — наша встреча: бесплатная, одна, "
        "как разговор один на один; отвечать можно словами или голосовыми, как "
        "удобнее.\n\n"
        "Начнём с фото. _Оно только для рисунка — я его не храню._",
        parse_mode="Markdown",
    )
